...
ohne_format:    lea     buffer,A0
                ...
                lea     buffer,A0
                bsr     executable      ;Bootsektor ausfÅhrbar machen
...
schreib_long:   moveq   #-1,D1          ;Alle Bits setzen
                ...
                lea     track_buffer,A0 ;ab hier Åbertragen
;                lea     hlpbuf,A0
;                lea     original,A0
                move.l  #40000,D1       ;LÑnge des Speicherbereichs
...
;Bringt File auf Amiga-Diskette und trÑgt ins Directory ein:
...
dir_loop:       move.b  (A0)+,(A1)+
                ...
                move.l  D0,D1           ;LÑnge der Datei in Bytes
                move.b  D1,28(A1)
                lsr.l   #8,D1
                move.b  D1,29(A1)
                lsr.l   #8,D1
                ...
                divu    #512,D0         ;Anzahl an Sektoren fÅr Datei
                addq.w  #1,D0           ;+1 als Rest

                add.w   D0,first_sek    ;nÑchste Schreibposition
                move.w  first_sek,D1    ;Ende der Datei
                subq.w  #1,D1           ;nur bis eins davor

                addi.l  #32,directory   ;nÑchsten Eintrag
                rts

;Macht einen Bootsektor ausfÅhrbar
...
;Dies ist ein ausfÅhrbarer Amiga-Bootsektor
boot:           DC.B 'DOS',0
checksum:       DC.L 0
root:           DC.L $0370
...
einfÅllen:      move.w  16(A6),D7       ;Startsektor
...
schreib_ende:   movem.l (SP)+,D0-A6
                move.w  D0,D5
                mulu    #512,D5         ;Anzahl zurÅckgeben
                rts
...
code_block:     movem.l D0-A6,-(SP)
                ...
                lsl.w   #2,D1
                or.w    #8,D1           ;Wert fÅr Blitterstart
...
message:        DC.B 27,"E"
                ...
                DC.B 'Ω 1991 by Till Bubeck',13,10,10
                DC.B 'Wenn die Disk auch formatiert werden soll, bitte "+" drÅcken. ',13,10,0

track_str:      DC.B 13,'Track: '
track_nr:       DC.B 'xxx',0
ver_error:      DC.B '    <- Verify Error!',13,10,0

amigatool:      DC.W 0          ;Kein Amigatool vorhanden (fÅr den Loader)
track_at_buffer:DC.W -1         ;Track im Buffer (ungÅltig)
first_sek:      DC.W 10         ;nÑchster Schreibstart (0..n)
