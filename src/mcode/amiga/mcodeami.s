...
;Maschinensprachemodul für Rings of Medusa II für Commodore Amiga
...
;Variablenexporte:
          ...
          .XDEF fast_len      ;Länge des Fast-Mem
...
aud0len   equ $000000a4       ;Länge in Worten
aud0per   equ $000000a6       ;Sample-Frequemz
aud0vol   equ $000000a8       ;Lautstärke

ciaapra   equ $00bfe001       ;Portregister A
potgor    equ $16   ;Für rechts Maustaste
...
amigastart:         move.w    #$2000,sr ;Alle IRQs ermöglichen
          ...
          move.l    $1c(a3),d7          ;Größe des BSS
          lsr.l     #4,d7     ;/8
          subq.l    #1,d7
          moveq     #0,d0
clear_bss:          move.l    d0,(a0)+
          move.l    d0,(a0)+
          move.l    d0,(a0)+  ;BSS löschen
          move.l    d0,(a0)+
          dbra      d7,clear_bss
...
;Überprüft, ob an angegebener Stelle 512 Kbyte Speicher ist
...
all_planes1:        move.l    a2,bltapth(a5)      ;Quelle A
          ...
          move.w    #150,d5   ;151 Zeilen einfügen
alle_1:   move.b    (a0),(a1) ;Am linken Rand reinkommen
          ...
          lea       -151*160-1(a0),a0   ;Zurück zum Start+1
...
mcode10:  link      a6,#0
          ...
          lea       17*160(a0),a0       ;Obere Leiste überspringen
...
mcode11:  link      a6,#0
          ...
          movea.l   8(a6),a0  ;Adresse des Quellbildes
          lea       168*160(a0),a0      ;Bild überspringen
...
;veränderte Register: keine
          ...
          moveq     #7,d7     ;Anzahl der Scanlines (Höhe des Sprites)
...
sub_col:  addq.l    #2,a0     ;Farbregister überspringen
...
mcode26:  movem.l   d0-d7/a0-a6,-(sp)
          ...
          lea       rasters2,a0
trans_copper:       move.w    (a0)+,(a2)+         ;Copperliste übertragen
...
farbe_schwarz:      addq.l    #2,a0     ;Farbregister skipen
          ...
          bra.s     trans1
skip:     addq.l    #4,a0     ;Befehl überspringen
          bra.s     trans1
...
addiere:  lea       rasters1,a0         ;Wird verändert
          lea       rasters2,a1         ;Endwert
such_raster:        cmpi.l    #$fffffffe,(a0)     ;Ende erreicht?
          beq.s     no_more_add
          cmpi.w    #$0180,(a0)         ;Raster gefunden?
          beq.s     auffaden
          addq.l    #4,a0     ;zum nächsten Befehl
          addq.l    #4,a1
          bra.s     such_raster
...
col_loop2:          addq.l    #2,a0     ;Farbregister überspringen
          addq.l    #2,a1     ;-"-
          move.w    (a0)+,d0  ;Farbe holen
...
schleife2:          
          ...
          clr.w     d0        ; Testwert
          clr.w     d1        ; Pos
          moveq     #1,d2     ; Counter für Bytewiederholungen
          clr.w     d3        ; HPos
          clr.w     d4
          clr.w     d5        ; Offset
          clr.w     d6        ; Lastpos
...
;Trägt in die Spriteliste der Maus ihre Koordinaten ein...
...
traf_err: moveq     #0,d3     ;Außerhalb der Karte=LEER
          bra.s     schultz_jetzt
...
mcode34:  movem.l   d0-d7/a0-a6,-(sp)
          ...
          bsr       set_mousexy         ;Trägt die Koordinaten ein
          ...
          bsr       rst22rst1 ;und zurückkopieren
...
new_i_3:  movem.l   d0-d7/a0-a6,-(sp)
          ...
          addq.l    #1,vbl_ct ;VBL-Counter erhöhen
          bsr       check_rand          ;überprüfen, ob Rand bewegt werden soll
          bsr       maus_auswert
...
maus_auswert:       moveq     #0,d0
          btst      #6,ciaapra          ;Links gedrückt?
          bne.s     kein_links
          moveq     #1,d0
kein_links:         move.w    potgor(a5),d1
          btst      #10,d1    ;rechts gedrückt?
          bne.s     kein_rechts
          addq.w    #2,d0
kein_rechts:        move.w    d0,mousek

          lea       oldmouse,a1
          move.w    joy0dat(a5),d0      ;Mauszähler lesen
          move.w    d0,d1
          lsr.w     #8,d1
          sub.b     (a1),d1
          ext.w     d1
...
;Überprüft, ob Spieler in der obersten Zeile mit links geklickt hat
...
check_rand:         tst.w     rand_bewegen        ;Wird Screen schon verschoben?
          ...
          bne.s     kein_rand
          cmpi.w    #3,mousek ;Rechts+Links gedrückt?
          bne.s     kein_rand

          clr.w     rand_bewegen        ;ab jetzt wird Screen bewegt!
          rts       

screen_move:        cmpi.w    #3,mousek ;Maustaste immer noch gedrückt?
          beq.s     rts78
...
;IRQ-Routine für Level 2 (CIA A/Expansion Port):
...
i_2_auswert:        move.w    (sp),d0   ;Kontrollregister holen
          ...
          ori.b     #$40,$00bfee01      ;für Handshake
...
buffer_voll:        
          ...
          move.w    #136,d0   ;Verzögerung für Amiga 2000-A
warten:   tst.w     $dff004   ;26 cc pro Schleife
          dbra      d0,warten ;ergibt 500 Mikrosekunden

          andi.b    #$bf,$00bfee01      ;Handshake zurück
...
;Diese Funktion spielt einen Digisound ab, dabei werden saemtliche benötigte
;IRQ's installiert, und wenn Digi fertig ist, werden die Routinen wieder
;destalliert. Wird als Startadresse 0L üübergeben wird evtl. vorhandene
...
round_round:        move.w    d0,anzahl_zeilen
          subq.w    #1,d0
          move.w    d0,anz_start        ;für nachher merken
          ...
          move.w    #64,aud0vol(a5)     ;volle Lautstärke

          move.w    #1,seq_counter      ;Sofort nachladen
          bsr.s     nxt_zeile ;Alle Parameter setzen

          move.w    #%0000000010000000,intreq(a5) ;Request löschen
          move.w    #$8201,dmacon(a5)   ;Audio-DMA ein
          move.w    #%1100000010000000,intena(a5) ;Kann IRQ auslösen
...
nxt_zeile:          movem.l   d0-d7/a0-a6,-(sp)
          ...
          move.w    #1,seq_counter      ;Beim nächsten Mal stoppen!
...
nicht_von_vorne:    moveq     #0,d5
          ...
          move.l    d0,digi_end
          moveq     #0,d5
          move.b    (a0)+,d5  ;Flag für Richtung
          ...
          move.l    digi_end,d0
          sub.l     digi_pos,d0         ;Länge berechnen
...
;Tabelle der verschiedenen Periodenwerte für die Samplefrequenzen:
...
;zu einem anderen. Dabei werden Überlappungen NICHT beruecksichtigt.
...
;Zielbreite < Quellbreite:
          subq.l    #2,a3     ;Maske eins früher starten
          subq.l    #2,a1     ;Ziel eins früher starten
...
fall_5:   addq.w    #2,quelle_breite    ;Fenster breiter machen
          ...
          subq.w    #2,a1     ;Aber ein Wort früher starten
          subq.w    #2,a3
...
; BLTSIZE berechnen (D3.W)
          ...
          addq.w    #1,d3     ; gibt Höhe in Zeilen
...
; Blitter-Register initialisieren
          ...
          neg.w     d0
          move.w    d0,bltamod(a5)      ;Immer zurück auf dieselbe Maske
...
;Tabelle zum Einfügen der Offsets direkt in den Programmcode
...
mirror_loop1:       move.w    (a1),d2   ;Rechten Rand merken
          ...
          subq.l    #2,a1     ;An Anfang des nächsten BB rechts
...
self_modify:        move.w    line_modus,d0
          ...
          moveq     #3,d7     ;4 Planes
put_word: moveq     #8,d0     ;weglöschen als Default
...
set_plane:          move.w    0(a0,d0.w),(a1)+    ;Opcode eintragen
          ...
          move.w    d1,(a1)+
nxt_plane:          addi.w    #40,d1    ;nächste Plane
          dbra      d7,put_word
...
;Setzt in der Copperliste die Adresse eines DMA-Adresszählers
;-> D1.W: Register, welches zu setzen ist (wird erhöht)
;   A0.L: zu schreibende Adresse
;   A2.L: Adresse der Copperliste (wird erhöht)
set_adr:  movem.l   d0/d2-d7/a0/a1/a3-a6,-(sp)
          ...
          move.w    d1,(a2)+
          move.w    d0,(a2)+
          addq.w    #2,d1     ;auf nächstes Register gehen
...
;-> A0.L: Adresse der neuen Farbpalette (wird erhöht)
;   A2.L: Adresse des Copperprogramms (wird erhöht)
...
;          andi.w    #%0000111011101110,d1         ;Unterstes löschen
;          move.w    d1,d2
...
mcode63:  link      a6,#0
          ...
          andi.b    #%10010101,d0       ;Inmode=00 -> Zählt Taktzyklen
          ori.b     #%00000001,d0       ;Start setzen
          move.b    d0,$00bfef01        ;Und Timer starten

          move.b    #%10000010,$00bfed01          ;ICR, Timer B kann IRQ auslösen
...
re_init:  move.w    12(a6),d0 ;Songnummer
          ...
          movea.l   music_routine,a0
          jsr       8(a0)     ;volle lautstärke       (100 = leise)
...
okay_hiller:        movem.l   (sp)+,d0-d7/a0-a5
          unlk      a6
          move.w    disk_error,d0       ;Fehlermeldung zurückgeben
          rts       
...
schreib_ende:       movem.l   (sp)+,d0-d7/a0-a6
          move.w    d0,d5
          mulu.w    #512,d5   ;Anzahl zurückgeben
          rts       
...
code_block:         movem.l   d0-d7/a0-a6,-(sp)
          ...
          lsl.w     #2,d1
          ori.w     #8,d1     ;Wert für Blitterstart
...
;Konvertiert ein Grafikobjekt in ein gerätespezifisches Format:
...
mcode71:  link      a6,#0
          ...
          move.w    6(a0),d7  ;Höhe des Objekts
          bra.s     neue_zeile
...
fast_len: .DS.l 1   ;Länge des Fast-Ram
...
mem_len:  .DS.l 1   ;Größe des freien Speichers
errno:    .DS.w 1   ;Globale Fehlernummer (für Library)

first_sek:          .DS.w 1
sek_per_track:      .DS.w 1
datei_laenge:       .DS.l 1   ;Die Länge der Datei in Bytes
...
maus_sprite:        .DS.w 34  ;für den Maussprite
...
end_rasters:                  ;Zum Berechnen der Länge
