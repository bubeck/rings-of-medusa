******************************************************************************
***                                                                        ***
***              Bunkerchecker V1.0 für Rings of Medusa II                 ***
***                                                                        ***
***                                                                        ***
***   ╜1991 by Till Bubeck, Ziegeleistr. 28, 7056 Weinstadt, 07151/66437   ***
***                                                                        ***
******************************************************************************
...
keine_null:     swap    D0
                ...
                move.l  D0,-4(A5)       ;und Länge merken
                adda.l  D0,A6
...
clr:            move.b  #-1,(A0)+       ;Alle Türen und Schlüssel unbenutzt
                dbra    D7,clr
...
check_bunker:   movea.l (A5)+,A0        ;Startadresse
                ...
                subq.l  #2,D6           ;2 Worte übersprungen
                addq.l  #4,A0           ;Größe überspringen
                move.w  D6,(A3)         ;Gesamtlänge eintragen

check_loop:     move.w  (A0)+,D0        ;Wert holen
                bne.s   kein_leerfeld
                addq.w  #1,2(A3)        ;ein Feld mehr
kein_leerfeld:  cmp.w   #DOOR_ZU_START,D0 ;Tür Nummer 1 nicht prüfen
                beq.s   nxt_wert
                bcs.s   keine_tür1      ;blt
                cmp.w   #DOOR_ZU_ENDE,D0
                bhi.s   keine_tür1      ;bgt

                sub.w   #DOOR_ZU_START,D0 ;Türnummer berechnen
check_tür:      tst.b   1(A1,D0.w)      ;Tür schon vorhanden?
                bpl     tür_doppelt
                move.b  D7,1(A1,D0.w)   ;Bunkernummer eintragen
                bra.s   nxt_wert

keine_tür1:     cmp.w   #DOOR_AUF_START,D0
                beq.s   nxt_wert
                bcs.s   keine_tür2
...
keine_tür2:     cmp.w   #KEY_START,D0   ;Schlüssel 1 nicht berücksichtigen
                beq.s   nxt_wert
                bcs.s   keine_key
                cmp.w   #KEY_ENDE,D0
                bhi.s   keine_key
...
such_ende:      tst.b   -(A3)
                ...
                lea     türen(PC),A1
                lea     schlüssel(PC),A2
check_lost_key: tst.b   (A1)            ;Tür vorhanden?
                bmi.s   tür_fehlt
                tst.b   (A2)            ;zugehöriger Schlüssel da?
                bpl.s   nxt_tür         ;ja->Weitermachen

                move.w  D7,D0           ;Schlüsselnummer
                lea     key_m1(PC),A4
                bsr     itoa
                lea     bun12(PC),A4
                move.b  (A1),D0         ;Schlüsselnummer
                bsr     itoa
...
nxt_tür:        addq.l  #1,A1
                addq.l  #1,A2
                addq.w  #1,D7           ;nächste Tür
                cmpa.l  A3,A1           ;Ende der Türen erreicht?
...
türen:          DS.B max_tuer   ;(-1=Unbenutzt, Sonst Bunkernummer)
schlüssel:      DS.B max_tuer   ;(-1=Unbenutzt; Sonst Bunkernummer)

bunker_start:   DS.L 60         ;Platz für 60 Bunker
bunker_len:     DS.L 60         ;-'-
