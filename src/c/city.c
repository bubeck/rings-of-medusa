...
void calc_gehalt()
{
  /* Berechnet die Summe der Einzelgehälter nach gesamt_gehalt */
  int i;
...
long sum_goods()
{
  /* Berechnet die Menge aller Dinge, die mitgeführt werden */
  long zahl;
...
long sum_waren()
{
  /* Summiert alle Waren auf, und gibt die Anzahl zurück */
  int i;
  long zahl;
...
int num;                /* Nummer der Stadt */
{
  ...
  leiste_oben(c_name[citynum]);
  redraw_buttons(PAUSE|EXIT_BTN|INFO_BTN|EDITOR);      /* Nur Veränderungen zeichnen */
  Sm();
}
...
void hafenkneipe()
{
  ...
  len=strlen(port_pub);             /* Alle rechten Spaces weglöschen */
  while(port_pub[--len]==' ') ;
  port_pub[len+1]=0;
  ...
  formular(scr1,63);                 /* Und Bildschirm löschen */
	show_raster();
}
...
int *index,nummer[];
{
  ...
  for(i=*index;i<SCHIFFMAX;i++)
    if (flotte[i].typ!=-1 && flotte[i].ort==citynum) {
      *index=i;                     /* Nächstes Schiff finden */
      nummer[flotte[*index].typ]++;      /* Nächste Nummer */
      return;
      }
  ...
  do {
    ...
    if(mk == 1) {
      ...
      if (my>88 && my<98) {       /* neuer Schiffstyp */
        x=75+ship_nr*80;
        drec(x,88,x+55,96);     /* Alte löschen */
        if (mx<155) ship_nr=0;
        if (mx>210) ship_nr=2;
      ...
      if (armee_status==AN_BORD ||
        ...
        if ((long)preis_einheiten[company]*chosen_men>money) money_alert();
          ...
          if (chosen_men!=0) {
            ...
            gehalt[company]+=(long)preis_einheiten[company]*chosen_men;         /* Total salary erhöhen */
...
void test_schulden()
{
  ...
  /* Wenn er Schulden seit länger als 3 Monaten hat, kann er nichts mehr kaufen */
...
void calc_zinsen()
{
  /* Berechnet die Zinsen, die fällig sind, seit Spieler das letzte Mal auf
     der Bank war */
  float geld;
  long tage;
  ...
  if (kontostand<0) {                /* Kredit */
    ...
    zinsen=geld;                     /* die fälligen Zinsen, hier negativ! */
    }
  if (kontostand>0) {
...
EINSATZ einsatz[];
{
	...
	do {
		...
		if (maus_in(CHIPX+27,CHIPY+11,CHIPX+37,CHIPY+11+9*6-1)) {
			...
			if (einsatz_nr==10) break;								/* max. 9 Einsätze */
...
void draw_roulette()
{
	...
	draw_grow_obj(0,pack_buf,0,scr1,15,73);						/* Window für Schüssel */
	copy_screen(scr1,scr2);
	feld_weiter();																		/* Rouletteschüssel zeichnen */
	switch_screens();
	copy_screen(scr1,scr2);
	show_raster();
	Sm();
...
int rotier_roulette()
{
	/* Dreht die Schüssel und lost eine Nummer aus */
	/* Gibt die ausgewählte Nummer zurück */
	int Nummer;																/* ausgewählte Nummer */
	...
	int kugelcounter;													/* zählt Kugelpositionen */
	int BahnDist;															/* Abstand der Kugel von oberen Rand */
	int KugelproBild;													/* Anzahl gedrawter Kugeln pro Schüsselbild */
	int x,y;																	/* Koordinaten für Kugelwackler */
	int i;
	FLAG aus;
	...
	while(Bildps<Endps) {											/* Beschleunigen */
		...
		Bildps+=Plusps;													/* Drehzahl erhöhen */
		} 
	Bildps-=Plusps;														/* richtiger Wert */
	Bildps/=0.4;
	Bildps=(int)Bildps*0.4;										/* damit Schüssel auf vollem Feld stehenbleibt */
	if (Nummer&1) Bildps+=0.2;
	
	kugelcounter=-4;													/* = relativer Mittelpunkt der Kugel */
	while(Bildps>1-0.2) {											/* Abbremsen */
		Nummer=feld_weiter();
		if (get_sync()/Bildps<=2*3)							/* Schüssel nicht wesentlich langsamer */
		...
		if (KugelproBild==0) {									/* keine Kugel */
			...
			/* um Kugellinie restaurieren zu können */
			...
			for(i=1;i<=KugelproBild;i++)	{   /*0,32,319,32+16+11,0,81+32*/
				if (Bildps>=4 || kugelcounter<=68+5) {				/* noch schnell oder Kugel noch sichtbar */
					if (Bildps>=16)									/* noch äußerste Kugelumlaufbahn */
						BahnDist=81+32;
				...
				cpy_raster(hlpbuf,scr2,0,0,319,objekt_hoehe(9,pack_buf)-1,0,81);	/* restore Schüssel */
				/* if (mousek) wait_key();				zu Prüfzwecken */
				}
			}
		Bildps-=0.2;														/* Verzögerung mit 0.2 Bildern pro Sekunde */
		}
	...
	return((int)roulette[Nummer/2]);								/* Feldnummer zurückgeben */
}

int feld_weiter()
{
	/* Geht ein Feld weiter und zeigt dieses an, gibt Feld_nr zurück */
...
int feld_nr;									/* index in roulette[] */
{
	...
	if (feld_nr&1) {									/* Zwischenposition */
		...
		switch (feld_nr) {
			case 1:									/* Grün auf Rot (0..1) */
				objekt=9;
				objekt1=30;						/* grüne Null */
				objekt2+=10;					/* -> rot */
				break;
			case 36*2+1:
				objekt=7;							/* Schwarz auf Grün (36..0) */
				objekt2=30;						/* grüne Null */
				break;
			default:
...
int num;                    /* Stadtnummer */
{
  ...
  mx=my=0;                              /* Damit in Stadt nichts angewählt wird */
  ...
  if (auf_schiff) {                 /* Er kam vom Seeweg in die Stadt */
    if (armee_status==AN_BORD) {
      ...
      if (armee_stadt==citynum) {           /* Ist Armee in dieser Stadt? */
        ...
        verlassen=TRUE;
        auf_schiff=TRUE;                    /* Er fährt jetzt Schiff! */
        button=EXIT_BTN;
        }
      else {
        alert(romstr156);
...
void land()
{
  ...
  if (auf_schiff) {
    if (armee_stadt!=citynum && armee_status==AN_LAND) {
      ...
      armee_stadt=citynum;              /* und zwar hier */
      sx[0]=sxalt[0]=sx[1];
      sy[0]=syalt[0]=sy[1];             /* Spritewerte übergeben */
...
void such_hafen()
{
  /* sucht den zugehörigen Hafen, zu der Stadt, in der Spieler ist */
  int x,y;
...
void such_eingang()
{
            /* sucht den zugehörigen Eingang, zu der Stadt, in der Spieler ist */
...
{
  int company;
  long costs;                   /* Kosten für die selektierten Prozent */
  long anzahl;                   /* Anzahl an SOldaten die selektiert sind */
  int selektion;                /* Was, Schild, Rüstung, oder Schwert */
  ...
  writexy_ausruestung(company);                /* Schreibt die Preise der Ausrüstung hin */
  Sm();
...
int company;
{
  /* Schreibt die Preise der Ausrüstung hin */
...
char txt1[],txt2[];
{
  ...
  strapp(zeile,player_name);               /* String anhängen */
...
int num;
{
  ...
  FLAG money_changed;                   /* Wurde das Geld verändert */
...
unsigned char wert;
{
  /* Zeitverzögerung beim Angriff */
  int dx;
	
  dx=((int)wert*80)/100;
  fill(1,x,y,x+dx,y+4);       			           		/* Roter Balken */
  if (wert!=100) fill(2,x+dx,y,x+80,y+4);           	  		/* Grüner Balken */
}
