...
int relativ;
{
	/* Zieht bei allen Reagenzgläsern eins ... */
	int member;
...
int relativ;
{
	/* Erniedert den Wasserstand einer Säule */
	int wert;
	char *tot;
	PARTY_MEMBER *partyp;
	int i;
	...
	if (!treffer_cheat) wert+=relativ;					/* Bei Cheat keine Veränderung mehr */
	if (wert<=0) {
		wert=0;
		if (member==0) {											/* Spieler? */
			switch (reagenz) {
...
int member;
{
	/* Überprüft, ob der Member der Party tot ist */
...
void draw_reagenz()
{
	/* zeichnet alle Reagenzgläser neu */
	int member;
	...
	copy_reagenz();											/* Kopiert Reagenzgläser auf 2. Screen */
	Sm();
}
...
{
	/* Zeichnet den Flüssigkeitsstand in einem Reagenzstand neu */
	...
	if (party[member].reagenz[rea]==0) {						/* Member ist tot? */
		...
		wasserhoehe=party[member].reagenz[rea]*hoehe/255;		/* Höhe der Flüssigkeit */
		if (wasserhoehe>hoehe) wasserhoehe=hoehe;
		
		if (wasserhoehe!=reagenz_stand[member*3+rea]) {			/* Nur bei Veränderung zeichnen */
...
void copy_reagenz()
{
	/* Kopiert die Reagenzgläser auf den 2. Screen */
...
int member;
{
  /* Zeichnet die Reagenzgläser einer bestimmten Person und beschriftet sie */
	...
	Hm();
	draw_obj(0,reagenz,MOVE,scr1,member*80,0);		/* Reagenzgläser zeichnen */
...
BUNKERWAND **waende;
{
	/* Trägt alle Items, die sich an dieser Stelle im Bunker befinden in die
		 Objektliste ein */
...
void do_enemy_shot()
{
	...
	if (shot.ob_mem!=NULL) {					/* Schuss vorhanden */
		if (shot.last_vbl==0) dif=shot.speed;		/* Erster Schuß sofort und ohne Zeit für draw_bunker() */
		else dif=vbl_ct-shot.last_vbl;				
		if (dif>=shot.speed) {					/* Weiterbewegen */
			shot.last_vbl=vbl_ct;
			dif/=shot.speed;							/* Wieviel Schritte überspringen? */
			shot.entfernung+=dif;							
			if (shot.entfernung>40) {
				...
				if (!betretbar(bunker[x][y])) shot.ob_mem=NULL;
					...
					if (x==bunker_x && y==bunker_y) {			/* Treffer am Spieler */
						blitz(0xfff);
						staerke=8+enemy[shot.gegner].speed/15;	/* Schussstärke Gegner 8..25 */ 
						for(i=0;i<PARTY;i++) 
							if (!is_tot(i)) {
	...
	if (shot.ob_mem==NULL)					/* Nur bewegen, wenn kein Schuss da */
      ...
      if (ep->bun_nr==(unsigned int)akt_bunker) {
				...
				if (differenz>ep->next_move) {
					...
					ep->fight=FALSE;						/* Feind kämpft nicht mehr */
					}
...
int item_nr;											/* Welches Item will der Spieler werfen -1=Keines */
{
	...
	int breite,hoehe;
	int links,rechts;
	static long old_schuss_vbl=0;				/* Wann darf der nächste Schuss kommen? */
	...
	if (item_nr>=0)										/* Objekt zum Werfen? */
		 if (item[item_nr].typ==KEY) key_throw=TRUE;	/* Schlüssel? */
	...
	if (!key_throw && vbl_ct>old_schuss_vbl+50) {	/* Nur einmal pro Sekunde */
		...
		if (item_nr>=0) {								/* Spieler will was werfen */
			...
			for(i=0;i<PARTY;i++) {
				...
				if (links!=-1 && rechts!=-1) 			/* Beide Hände voll */			
					if ((item[links].typ==WAFFE && item[rechts].typ==MUNITION) ||
						...
						if (item[links].typ==MUNITION) {
							if (item[links].flag>0) {				/* Munition da? */
								...
								else kann_schiessen=TRUE;				/* er könnte Schiessen */
								}
							}
						else 
		...
		if (schussstaerke>0) {							/* Es war noch ein Schuß da... */
      ...
      sync_50_60=1;
			...
			if (item_nr<0) {								/* Richtiger Schuss */
				while(helligkeit<6) {						/* Schuß volle Helligkeit */
					helligkeit++;
					set_bunker_pal();
					}
				if (oldhelligkeit<=4) {
...
{
	/* Überprüft, ob an dieser Stelle ein Gegner bzw. Wand ist. */
	register ENEMY *ep;
	register int i;
	int x,y;
	int size;
...
{
	/* Kopiert Screen1 auf Screen2 und paßt die Automapvariablen an */
...
void medusa()
{
	/* Der Spieler hat die letzte Tür des Bunkers geöffnet -> Medusa kommt */
	int animation;
...
int med_nr,kug_nr;
{
	...
	Hm();
	cpy_raster(hlpbuf,scr2,0,45,319,167,0,45);			/* weglöschen */
