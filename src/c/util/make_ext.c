...
void main()
{
  ...
  rpos=0; wpos=0;
	...
	write_line("   ╜ 1991 by Till Bubeck, Ziegeleistr. 28, 7056 Weinstadt */");
	write_line("");
  ...
  if ((fh_w=Fcreate("EXTERN.C",0))<0) {
	...
	z1=skip_space();
  nachher=z1;                   /* nachher nochmal zurückgeben */
...
char skip_space()
{
  ...
  do {                  /* Führende Spaces etc. überspringen */
    z1=get_zeichen();
...
char lies_zeichen()
{
  /* liest alle zeichen, aber überspringt '='! */
  char zeichen;
  ...
  if (nachher!=0) {                 /* Nächstes Zeichen schon gelesen? */
    zeichen=nachher;                /* ja, nicht lesen, zurückgeben */
    nachher=0;
    }
  else {
...
char get_zeichen()
{
  /* Liest, und überspringt Kommentare und { } (geschachtelt) und = */
  char z1;
	int tiefe;
	
  z1=lies_zeichen();
  if (z1=='/') {
    nachher=lies_zeichen();          /* nächstes Zeichen holen und sichern */
  ...
  if (z1=='{') {
    ...
    return(get_zeichen());          /* Klammern übersprungen, weitermachen */
    }
	if (z1=='=') {
		do {											/* Ab '=' bis zum ';' übersrpingen */
			z1=lies_zeichen();
			} while(z1!=';');
		}
