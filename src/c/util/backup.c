...
/* Version 1.0 */
/* Backup Accessory - speichert alle veränderten Files auf Harddisk */
...
int drv_map;                /* mögliche Laufwerke */
...
long memory;
long max_len;               /* maximale Länge des Speichers */
...
main()
{
  ...
  drv_map=Drvmap();         /* alle möglichen Laufwerke holen */
  if ((drv_map&16==16))
  ...
  if (Fsfirst("*.*",20)>=0)               /* Alle Files kopieren */
    do {
      if (dta_loc.dta_attr==16) {         /* Ordner */
        ...
        hier_pfad[len+1]=0;
        backup(hier_pfad);
        Dsetpath(quellpfad);                 /* und wieder schließen */
        }
...
dta dta_act;
{
  ...
  if (time<dta_act.dta_time) {      /* File muß kopiert werden */
