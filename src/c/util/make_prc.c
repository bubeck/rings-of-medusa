		...
		/* Preisberechnung für Rings of Medusa II 
			 ╜ by Till Bubeck (3.6.1991)	*/
...
#include "o:\grund.c"              /* Konstantendefintion der Untergründe */
...
int prod[CITIES][5] = {     
	...
	6,0,0,0,0,											/* C0 Fische */
	14,0,0,0,0,											/* E0 Förderturm */
	19,0,0,0,0,											/* F0 Computer */
	20,0,0,0,0,											/* F1 Chips */
	7,0,0,0,0,											/* F2 Waffen */
	1,0,0,0,0,											/* F3 Milch */
	13,0,0,0,0,											/* G0 Sklaven */
	2,3,0,0,0,											/* H0 Käse, Salat */
	4,0,0,0,0,											/* H1 Äpfel */
...
void main()
{
  /* Berechnet die Preise in den einzelnen Städten, je nach Entfernung und
     Produkt, das die Städte produzieren */
  int fh;
...
void calculate()
{
  ...
  for(std=0;std<CITIES;std++) {
    ...
    for(waren=1;waren<=WAREN+STALLWAREN+ROHSTOFFE;waren++) {
      ...
      /* Prüfen, ob selber produziert wird */
      for(i=0;i<5;i++)
      ...
      if (i!=5) {                           /* Wird selber produziert */
        ...
        /* nächste Stadt suchen, in der Ware produziert wird */
...
void get_koord()
{
  /* Holt sich die Koordinaten der Städte */
  int land;
  int x,y;
  int nr;
  int pos;
...
void *name,*adr;
{
  ...
  if (fh>=0) {
	...
	else printf("Fehler! Datei %s kann nicht geöffnet werden!",name);
}
...
int land,x,y;
{
  ...
  if (anzahl>=0) {                              /* ist voll in op1 */
    ...
    op2>>=8+anzahl;                             /* Achtung, anzahl ist negativ! */
                /* ^^^ LASER_C verwendet ASR, d.h. obere Bits löschen */
    op2&=(1<<(-anzahl))-1;
    wert|=op2;
    }
