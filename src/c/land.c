...
/* Dieser Part beinhaltet alle Funktionen, die benötigt werden, damit der
   Spieler sein Sprite auf der Landkarte bewegen kann... */
/* Achtung, alle Stellen, die mit !!! markiert sind, sind zu Debug-zwecken geändert worden! */

#include "includes.c"           /* Definiert alle Variablen als Extern,... */

long timecounter;								/* Counter zum Zählen der Tage */
...
int x,y;            /* X/Y des Spielers, damit nicht ins Wasser gezogen wird */
{
  ...
  if (gesperrt) {                   /* Ist Grund begehbar? */
    ...
    load_map(welt[pos_welt]);       /* Altes Land laden */
		sx[0]=sxalt[0];
		sy[0]=syalt[0];							/* Spieler zurücksetzen */ 
    get_ground();
    Sm();
    return(FALSE);
    }
  ...
  copy_screen(scr1,scr2);         /* auf 2. Screen übertragen */
...
int nr;
{
  ...
  if (nr==SEA) {
    for(i=0;i<1800;) {
      ground_buf[i++]=255;		/* Ganze GMAP mit Wasser füllen (packed) */
      }
    }
  else {
		switch (nr) {
...
void del_rohstoffe()
{
	/* Da viel zuviele Rohstoffe vorhanden sind, werden abhängig vom Level einige wieder
	   entfernt. */
...
{
  /* Ändert in der aktuellen GMAP den Untergrund an bestimmten Stellen */
  int i,j;
	int untergrund;
    ...
    if (new_ground[i].land==nr) {
	      ...
	      put_raster(new_ground[i].untergrund,new_ground[i].x,new_ground[i].y);
				...
				for(j=i;j<ground_nr;j++) 
					new_ground[i]=new_ground[i+1];			/* Eintrag unnötig -> entfernen */
				i--;													/* und zurück */
				}
			}
}
...
{
  /* Ändert das Land an dieser Stelle, und trägt es in die new_ground[]-Liste
     ein */
  register int i;
  register int xr,yr;
...
void clear_time()
{
  /* Löscht die interne Uhr, (VBL_COUNTER) */
...
void check_time()
{
  /* Überprüft, ob 1 Tag um */

  if (timecounter+70<vbl_ct) {               /* Alle 70/50 sec. ein Tag um */
    clear_time();               /* Uhr wieder löschen */
...
void test_mine()
{
  /* Überprüft, ob der Spieler in eine Mine will */
  register int i;
  ...
  sx[0]=sxalt[0];           /* Spieler zurück setzen */
  sy[0]=syalt[0];
  get_ground();
  show_ground();
}
...
void test_city()
{
  ...
  if ( (!auf_schiff && ground_num>=STADT_1_EIN && ground_num<=STADT_4_EIN)
    ...
    if (belong[citynum]==99) {                          /* Diese Stadt gehört Medusa */
      if (auf_schiff && armee_status!=AN_BORD)          /* Armee an Bord? */
        redraw_buttons(NO);                             /* Nein, kein Angriff möglich */
      else redraw_buttons(NO|ATTACK);
      show_window(romstr589);
      }
    else {                                              /* Stadt gehört xxx */
      if (belong[citynum]>=0) {                         /* Stadt gehört Gegner */
        for(i=0;i<GEGNER;i++)
        ...
        if (i==GEGNER) {                                /* normale Stadt */
          if (auf_schiff && armee_status!=AN_BORD)          /* Armee an Bord? */
            redraw_buttons(YES|NO);                             /* Nein, kein Angriff möglich */
  ...
  if (ground_num>=BUNKER_1 && ground_num<=BUNKER_2) {
    bunker_num=bunker_nr[map][ground_num-BUNKER_1];
		if (bunker_num>=0 && bunker_num<128 && bunker_bekannt[bunker_num]) {
			sx[0]=sxalt[0];								/* Spielfigur zurücksetzen */
			sy[0]=syalt[0];
...
void such_platz()
{
  /* Sucht einen freien Platz für den Spieler, also keine Berge, Wasser ,.. */
...
{
  /* Überprüft, ob der Spieler sich im Sumpf oder verhexten Wald befindet,
     dann sterben nämlich die Leute */
  FLAG gestorben;
  int i;
  long tot;
...
int xr,yr;
{
  /* Ändert den Untergrund in der GMAP an Rasterstelle xr,yr */
...
int x,y;                    /* In Pixelkoordinaten */
{
  ...
  op2_int&=0x00ff;                              /* ABer nur unterstes Byte gültig */

  wort=(op1<<8) | op2_int;                      /* Beide Bytes als Wort */

  loesch=0xffe0;                                /* Die untersten 5 Bit gelöscht */
  for(i=0;i<bit;i++) {
  ...
  wort&=loesch;                     /* Alten Wert löschen */
...
int x,y;
{
  ...
  if (x<0 || x>319 || y<21 || y>168) {
    internal(romstr597);    /* außerhalb */
    }

  if (y<25) return(EBENE);                  /* Außerhalb ist Ebene */
  ...
  if (anzahl>=0) {                              /* ist voll in op1 */
    ...
    op2>>=8+anzahl;                             /* Achtung, anzahl ist negativ! */
                /* ^^^ TURBO_C verwendet ASR, d.h. obere Bits löschen */
    op2&=(1<<(-anzahl))-1;
    wert|=op2;
    }
...
int x,y;
{
  ...
  /* ist X/Y außerhalb, gibt Funktion -1 zurück */
...
void update_coordinates()
{
  ...
  if (my <= 24) my = 24;
  if (mx>=311) mx=311;
  if (my >= 167) my = 167;      /* Mauskoordinaten so verändern, daß nicht
  ...
  if (newpos!=-1) {                 /* Grenze erreicht */
    if (land_map(welt[newpos],TRUE,rich,newx,newy)) {   /* Laden und anzeigen */
      ...
      sxalt[0]=newaltx;
      syalt[0]=newalty;
      pos_welt=newpos;                        /* Neue Position übernehmen */
...
int wert;
{
  /* Testet, ob ein Untergrund übergeben wurde, der beschiffbar ist */
...
int x,y;
{
  /* Überprüft, ob an diesen Koordinaten ein Fluß liegt, und zieht ggf. hin */
...
void test_ground()
{
        /* Ermittelt Untergrund für Spielersprite */
	register int x,y;
	int schild;
  ...
  if (auf_schiff) {                 /* Spieler will Schiff bewegen */
    ...
    switch(ground_num) {
        ...
        case WASSER:
				...
				case VULKANE:
        case HOHE_BERGE:
                 sx[0]= sxalt[0];			/* Figur zurücksetzen */
                 sy[0]= syalt[0];
                 get_ground();
                 break;
				case SCHILD:
...
int nr;
{
	...
	for(;nr>0;nr--) {
		...
		while(*poi==10 || *poi==13) poi++;					/* Trennzeichen überspringen */
		}
	start=poi;
	while(*poi!=13 && *poi!=10) poi++;					/* Stringende suchen */
	*poi=0;																/* EOS setzen */
...
void schiff_test()
{
  /* Überprüft, ob in einer Stadt ein Schiff fertig gebaut ist */
  register int i,j;
  int index;
...
int nr;
{
  /* In steuer[nr] muß die Startstadt eingetragen sein, dann berechnet diese
...
int nr;
{
  /* Bestimmt einen neuen Wert für aktweg, der aber nicht der Alte sein darf
  ...
  if (anzahl==1) steuer[nr].akt_weg=-1;     /* Wenn nur ein Weg, dann alle gültig */
...
int nr,anzahl;
{
  /* Sucht nach einem neuen Weg, wobei anzahl Wege zur Verfügung stehen
...
void set_armies()
{
  ...
  for(i=0;i<GEGNER;i++) {
    do {
	    ...
	    steuer[i].heimat=rnd; 		         		/* Heimatstadt */
			steuer[i].akt_weg=-1;					/* Jeder Weg ist gültig */
      } while(belong[steuer[i].heimat]!=99);
    belong[steuer[i].heimat]=i;         	  /* Stadt gehört dieser Armee */
  ...
  army_unten=TRUE;
  move_armies();                /* Untere Hälfte bewegen */
  move_armies();                /* Und jetzt die obere Hälfte */
}
...
void move_armies()
{
  ...
  if (army_unten) {
    army_unten=FALSE;           								/* Die Untere Hälfte bewegen */
    start=0;
    ende=GEGNER/2;
    }
  else {
    army_unten=TRUE;            								/* Die obere Hälfte bewegen */
    start=GEGNER/2;
    ende=GEGNER;
    }
...
register int nr;
{
  ...
  if (steuer[nr].x % 4==2 && (steuer[nr].y-25) % 4==2) {    /* Mitte des Kastens */
    ...
    neue_richtung(nr);              /* Ermittelt neue Richtung für Armee */
    }
...
register int nr;
{
  ...
  such_weg(nr);                     /* neuen Weg auswählen */
}
  ...
  for(i=0;i<GEGNER;i++) {
    if (steuer[i].weltpos==pos_welt) {            		/* Gegner ist in diesem Land */
      ...
      if (abstand<armeeteil[0][3]/3 || pio_cheat) anschalten=TRUE;
			else if (abstand<8) anschalten=TRUE;				/* ein bißchen sieht man immer */
...
int nr;
{
  ...
  if (tw<=TSTADT_4) {                                     /* Stadt? */
    ...
    if (belong[stadt_nr]<0 && armeegesamt[nr+1]>50) {  /* Gehört die Stadt Spieler? */
      stadt_gesamt=0;
    ...
    if (belong[stadt_nr]==nr) {         /* Gehört die Stadt dieser Armee? */
      stadt_gesamt=0;
      for(i=0;i<EINHEITEN;i++) stadt_gesamt+=stadt_armee[stadt_nr][i];
      ...
      if (!steuer[nr].auf_wasser) {       /* Armee war an Land */
        if (zufall(5)==3) {               /* Nur manchmal */
          ...
          if (such_ort(nr,wert)) {        /* Ort gefunden */
            steuer[nr].akt_weg=-1;        /* Jeder Weg ist gültig */
...
int nr;
{
  /* Armee befindet sich in der Mitte eines Kastens, nun muß eine neue
    Zugrichtung gesucht werden, wobei sie nicht die Alte sein darf,
    da die Armee ja dann rückwärts ziehen würde. Dabei wird akt_weg benützt. */
  ...
  for (yy=ry-1;yy<=ry+1;yy++)
    for (xx=rx-1;xx<=rx+1;xx++)        /* 9 Kästchen drum'rum prüfen */
  ...
  /* Es wurde kein entsprechender Weg gefunden, also muß nach einer */
...
void make_world()
{
  ...
  for(i=0;i<225;i++) welt[i]=SEA;           /* Am Anfang war überall Wasser... */
...
int x,y;
{
  /* Holt aus welt[] die Landschaft unter Berücksichtigung der Kugerlform */
